...
# Создаём словарь, значения ключей которого будут наши шаблоны.
int_mode = {'access': access_template, 'trunk':trunk_template}

# Вводим режим работы порта. Введёное значение будет являться ключом словаря.
mode = int_mode.get(input('Enter interface mode (access/trunk): '))
interface = input('Enter interface type and number: ')
vlan = input('Enter vlan(s): ')

print('\ninterface {}'.format(interface))
print('\n'.join(mode).format(vlan))